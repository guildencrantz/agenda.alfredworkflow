  ...
  return {
    get: function(todayOnly, listName, limit) {
      ...
      eventData.forEach(ri => {
        ...
        findURI = `find/`+encodeURIComponent(parsed.calendar)+`/`+encodeURIComponent(parsed.title)+`/`+eventTSisoSeconds

        busycalURI = `busycalevent://${findURI}`
        ...
        {
          console.log("Appending meeting URL")
        ...
        items.push({
          title: parsed.title,
          subtitle,
          //arg: $.getenv('use_busycal') == true ? `c:${busycalURI}` : `c:${eventStartDate}`,
          //arg: `c:${findURI}`,
          arg: arg,
